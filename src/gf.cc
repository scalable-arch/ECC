...
{
    // From page 30 of "Code design for dependable systems" / Eiji Fujiwara / Wiley
    if (m==1) {
        ...
        return POLY(0x1100Bu);  // x^16 + x^12 + x^3 + x + 1 = 0  -> {1_0001_0000_0000_1011} -> 0x1100B
    } else {
...
POLY GF<p, m>::genMinimalPoly(int alphaPower) {
    ...
    for (int i=0; i<m; i++) {
        int alphaIndex = (alphaPower<<i) % ((1<<m) - 1);    // alpha^((1<<m)-1) is 1
    ...
    coeff = 0;
    for (set<int>::iterator it = conjugates.begin(); it != conjugates.end(); ++it) {
        for (set<int>::iterator it2 = conjugates.begin(); it2 != conjugates.end(); ++it2) {
            if (*it > *it2) {
    ...
    for (set<int>::iterator it = conjugates.begin(); it != conjugates.end(); ++it) {
        for (set<int>::iterator it2 = conjugates.begin(); it2 != conjugates.end(); ++it2) {
            for (set<int>::iterator it3 = conjugates.begin(); it3 != conjugates.end(); ++it3) {
                if ((*it > *it2) && (*it2 > *it3)) {
    ...
    for (set<int>::iterator it = conjugates.begin(); it != conjugates.end(); ++it) {
        for (set<int>::iterator it2 = conjugates.begin(); it2 != conjugates.end(); ++it2) {
            for (set<int>::iterator it3 = conjugates.begin(); it3 != conjugates.end(); ++it3) {
            for (set<int>::iterator it4 = conjugates.begin(); it4 != conjugates.end(); ++it4) {
    ...
    for (set<int>::iterator it3 = conjugates.begin(); it3 != conjugates.end(); ++it3) {
    for (set<int>::iterator it4 = conjugates.begin(); it4 != conjugates.end(); ++it4) {
    for (set<int>::iterator it5 = conjugates.begin(); it5 != conjugates.end(); ++it5) {
    ...
    for (set<int>::iterator it4 = conjugates.begin(); it4 != conjugates.end(); ++it4) {
    for (set<int>::iterator it5 = conjugates.begin(); it5 != conjugates.end(); ++it5) {
    for (set<int>::iterator it6 = conjugates.begin(); it6 != conjugates.end(); ++it6) {
        if ((*it > *it2) && (*it2 > *it3) && (*it3 > *it4) && (*it4 > *it5) && (*it5 > *it6)) {
            int newIndex = ((*it) + (*it2) + *(it3) + *(it4) + *(it5) + *(it6)) % maxIndex;
            coeff ^= index2poly(newIndex);
        }
    }
    }
    ...
    for (set<int>::iterator it5 = conjugates.begin(); it5 != conjugates.end(); ++it5) {
    for (set<int>::iterator it6 = conjugates.begin(); it6 != conjugates.end(); ++it6) {
    for (set<int>::iterator it7 = conjugates.begin(); it7 != conjugates.end(); ++it7) {
        if ((*it > *it2) && (*it2 > *it3) && (*it3 > *it4) && (*it4 > *it5) && (*it5 > *it6) && (*it6 > *it7)) {
            int newIndex = ((*it) + (*it2) + *(it3) + *(it4) + *(it5) + *(it6) + *(it7)) % maxIndex;
            coeff ^= index2poly(newIndex);
        }
    }
    }
...
template <int p, int m>
GFElem<p, m>& GFElem<p, m>::operator+=(const GFElem& rhs) {
    indexValue = gf.poly2index(gf.index2poly(indexValue) ^ gf.index2poly(rhs.indexValue));
    return *this;
}
...
GFPoly<p, m>& GFPoly<p, m>::operator*=(const GFPoly<p, m>& rhs) {
    int maxDegree = (degree > (degree+rhs.degree)) ? degree : (degree+rhs.degree);
    GFElem<p, m>* newCoeffArr = new GFElem<p, m>[maxDegree+1];
    for (int i=degree; i>=0; i--) {
